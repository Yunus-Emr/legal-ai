...
class RetrievalService:
    ...
    async def search(
        ...
        return await self.search_hybrid(
            query=query, top_k=top_k, filter_doc_ids=filter_doc_ids, db=db
        )
    ...
    async def search_hybrid(
        ...
        try:
            ...
            return await self._enrich_hits_with_metadata(hits, db=db)
        except Exception as e:
            logger.warning(f"[Retrieval] OpenSearch Hybrid bağlanamadı: {e}. FAISS kNN fallback")
            return await self.search_by_vector(vector=vector, top_k=top_k, filter_doc_ids=filter_doc_ids, db=db)
    ...
    async def _enrich_hits_with_metadata(
        self,
        ...
        if db is not None:
            ...
            # Fallback: mevcut oturum yoksa yeni aç (dokuman indexleme sırasında)

"""
╔══════════════════════════════════════════════════════════════════╗
║         TÜRK HUKUKİ DOKÜMAN CHUNKER v3.0                        ║
...
class Patterns:
    """Tüm regex pattern'ları tek yerde — compile once, use everywhere."""
    ...
    # "Kanun No: 6331" veya "6331 sayılı Kanun"
    ...
    # Birleşik header splitter — sadece bir kere build ediliyor
...
@dataclass
class ArticleBlock:
    """split_by_articles çıktısı — ham blok."""
    article: str
...
class Chunk:
    ...
    def to_record(self) -> dict:
        """JSONL satırı için düz dict — metadata iç içe değil, flatten."""
        base = {
...
# ══════════════════════════════════════════════════════════════════
# NER — STRATEGY PATTERN
# ══════════════════════════════════════════════════════════════════

class NERStrategy(ABC):
    """Soyut strateji — tüm NER uygulamaları bu arayüzü takip eder."""
...
class RegexNERStrategy(NERStrategy):
    """Transformer yoksa devreye giren kural tabanlı fallback."""
...
class TransformerNERStrategy(NERStrategy):
    """
    savasy/bert-base-turkish-ner-cased tabanlı NER.
    Model lazy olarak ilk `extract()` çağrısında yüklenir.
    """
    ...
    def __init__(self, model_name: str, device: str = "cpu") -> None:
        ...
        self._failed     = False         # kalıcı hata bayrağı
    ...
    def extract(self, text: str) -> dict[str, list[str]]:
        ...
        try:
            # İlk ~600 kelime, 2000 karakter → model 512-token limiti
...
class PDFReader:
    """Sayfa bazlı ham metin çıkarımı."""
...
class TextCleaner:
    _RULES = [
        ...
        (re.compile(r"\t"),      " "),   # tab
        (re.compile(r"\s{2,}"),  " "),   # fazla boşluk
    ]

    # Dipnot: satır sonu rakam ya da rakam+nokta (ör: "yer:34 34 14/10/2021…")
    # PDF'lerde dipnot referansı genellikle şu kalıplarda gelir:
    #   "kelime34"  →  kelime + dipnot no
    #   satır başında tek/çift rakam + boşluk + uzun metin
    ...
    def remove_footnotes(cls, text: str) -> str:
        """
        Chunk metninden dipnot referanslarını ve dipnot gövdelerini siler.
        Örnek:
          "…yer:34 34 14/10/2021 tarihli ve 7338 sayılı Kanunun 17…"
          →  "…yer:"
        """
        text = cls._FOOTNOTE_BODY.sub("", text)   # önce gövdeyi sil
        text = cls._FOOTNOTE_REF.sub("", text)    # sonra referans numarasını
...
class MetadataExtractor:
    ...
    def _normalize_entities(items: list[str]) -> list[str]:
        """
        Whitespace varyasyonlarından kaynaklanan tekrarları temizler.
        "Maliye   Bakanlığı" ve "Maliye \\nBakanlığı" → "Maliye Bakanlığı"
        """
        seen:   set[str]  = set()
        result: list[str] = []
        for item in items:
    ...
    def _kanun_no_from_filename(filename: str) -> Optional[str]:
        """
        PDF adı '1.4.213.pdf' formatındaysa son segmenti kanun no olarak al.
        Yalnızca regex metadata null döndürdüğünde kullanılır.
        """
...
class ArticleSplitter:
    """
    MADDE / KARAR bazlı bloklara ayırır.
    `flush` iç sınıf ile state yönetimi nonlocal'dan kurtarıldı.
    """

    def __init__(self, config: ChunkerConfig) -> None:
        self._min_words = config.min_chunk_words

    # ── yardımcı: parça bir başlık mı? ──────────────────────────
    @staticmethod
...
class DirectChunkingStrategy(ChunkingStrategy):
    """Kısa bloklar → doğrudan tek chunk."""
...
class SlidingWindowStrategy(ChunkingStrategy):
    """Uzun bloklar → örtüşen pencereler."""
    ...
    def chunk(self, block: ArticleBlock, metadata: DocumentMetadata) -> list[Chunk]:
        ...
        for idx, win in enumerate(windows):
            # Her pencereye düşen sayfa dilimini hesapla (yaklaşık)
            start_p   = idx * pages_per_win
            end_p     = start_p + pages_per_win + 1   # +1 overlap
...
class HybridChunker:
    """
    Blok uzunluğuna göre strateji seçer.
    Yeni strateji eklemek için sadece ChunkingStrategy implement et.
    """
...
# ══════════════════════════════════════════════════════════════════
# PDF PROCESSOR  (tek PDF için orkestrasyon)
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# PIPELINE  (tüm dataset için orkestrasyon)
# ══════════════════════════════════════════════════════════════════
...
class ChunkerPipeline:
    def __init__(self, config: ChunkerConfig) -> None:
        ...
        # Bağımlılık ağacını burada kur — tek yer
        ner_strategy = (
